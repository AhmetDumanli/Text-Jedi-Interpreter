...
void yorumKontrol(char* str) {
    ...
    if (startPtr != NULL) { // Başlangıç ifadesi bulunduysa
        if (endPtr != NULL) { // Bitiş ifadesi de bulunduysa
            printf("Yorum ifadesi tamamlandı.\n");
        } else {
...
void setVariable(struct Variable* var, const char* first, const char* second, const char* third) {
    ...
    if (plus != NULL) {
        ...
        if(isNumeric(leftOperandPtr) == 0){
            ...
            for(int i = 0; i < degisken_sayisi; i++){
                if(strcmp(variables[i].name, leftOperandPtr) == 0 && strcmp(variables[i].type, "int") == 0){
        ...
        trimLeadingSpaces(rightOperandPtr);
        if(isNumeric(rightOperandPtr) == 0){
            for(int i = 0; i < degisken_sayisi; i++){
                if(strcmp(variables[i].name, rightOperandPtr) == 0 && strcmp(variables[i].type, "text") == 0){
...
void calculateSubtraction(char* str) {
    // - işaretini bulma
...
void processString(const char* string) {
    ...
    if (strcmp(leftToken, rightToken) < 0) {
        printf("Hatali giris: Ilk ve son karakterler cift tirnak isareti olmalı.\n");
        strcpy(string, "");
        return;
    }
...
void locateFunction(char* string, int size, struct Variable variables[]) {
    ...
    snprintf(string, size, "%d", indexFound);
}
...
void overrideFunction(char* string, int size, struct Variable variables[]) {
    ...
    while (*p != ')' && *p != '\0') {
        ...
        if (*p == ',') {
            ...
            if (resultCount == 1) {
                ...
                if (!found) {
                    strcpy(valueOfVariable3, result);
                    printf("3.parametre :%s\n", valueOfVariable3); // Eşleşme bulunamazsa direkt değeri al
                }
            }
        } else {
            result[index] = *p;
    ...
    strncpy(temp, valueOfVariable1, start);
    temp[start] = '\0';
    printf("Yeni string değeri :%s\n", temp);
    // valueOfVariable3'ü sonuna ekle
    strcat(temp, valueOfVariable3);
    printf("Yeni string değeri :%s\n", temp);
    strcpy(string,temp);
}
...
void asString(char* str, int size, struct Variable variables[]) {
    ...
    if (strcmp(variables[i].type, "int") != 0) {
        printf("Hata: %s değişkeninin tipi int değil!\n", varName);
        return;
    }
    ...
    printf("asString ifadesi başarıyla güncellendi: %s\n", str);
}
...
void asText(char* str, int size, struct Variable variables[]) {
    ...
    printf("asText ifadesi başarıyla güncellendi: %s\n", str);
}
    ...
    for (int i = 0; dizi[i] != NULL; i++)
    {
        ...
        if(startsWithKeywords(dizi[i])){
            ...
            if(strcmp(kelimeDizisi[0],"new") == 0){
                setVariable(&variables[degisken_sayisi], kelimeDizisi[0], kelimeDizisi[1], kelimeDizisi[2]);
                degisken_sayisi ++;
            }
                ...
                for(int i = 0; i < size2; i++){
                    if(strcmp(kelimeDizisi[1], variables[i].name) == 0 && strcmp(variables[i].type, "text") == 0){
                        strcpy(variables[i].value, dosyaOku(kelimeDizisi[3]));
                        printf("%s\n", variables[i].value);
                    }
                }
                ...
                for(int i = 0; i < size3; i++){
                    if(strcmp(kelimeDizisi[1], variables[i].name) == 0 && strcmp(variables[i].type, "text") == 0){
                        stringDosyayaYaz(variables[i].value, kelimeDizisi[3]);
                        printf("%s\n", variables[i].value);
            ...
            if(strstr(subStr2, "subs(") != NULL){
                ...
                subsFunction(subStr2, size, variables);
            }
            if(strstr(subStr2, "locate(") != NULL){
                locateFunction(subStr2, size, variables);
            }

            if(strstr(subStr2, "insert(") != NULL){

                insertFunction(subStr2, size, variables);
            }
            if(strstr(subStr2, "override(") != NULL){
                overrideFunction(subStr2, size, variables);
            }
            if(strstr(subStr2, "asString(") != NULL){
                asString(subStr2, size, variables);
            }
            ...
            for(int k = 0; k < degisken_sayisi; k++){
                if(strcmp(subStr1, variables[k].name) == 0){
                    ...
                    if(strcmp(variables[k].type, "int") == 0 && isNumeric(subStr2) == 0){
                        ...
                        printf("Negatif degerler desteklenmiyor!");
                        exit(1);
                    }
                    ...
                    if(find == 0){
                        printf("Boyle bir degisken tanimlanmamis!");
                    }
